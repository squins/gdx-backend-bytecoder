...
//	  // DISABLED: Performance  System.out.println("TAG: " + tag + " message: " + s);
	...
	int locateKey (K key) {
		gdxAppError("ObjectMap", "LocateKey, key: " + (key ==null));
...
//			gdxAppError("ObjectMap","inside for loop");
			...
			if (other == null) {
				gdxAppError("ObjectMap", "return this, because empty space available:" + (-(i + 1)));
				gdxAppError("ObjectMap", "Other is null");
...
//			gdxAppError("ObjectMap", "key.type: "+ (key.getClass().getName()));
			...
			gdxAppError("ObjectMap","after for loop finall");
		}
	}

	/** Returns the old value associated with the specified key, or null. */
	public @Null V put (K key, @Null V value) {
		gdxAppError("ObjectMap","put (K key, @Null V value)");
		int i = locateKey(key);
		gdxAppError("ObjectMap","after int i = locateKey(key);");
		if (i >= 0) { // Existing key was found.
			gdxAppError("ObjectMap","Existing key was found.");
			V oldValue = valueTable[i];
			valueTable[i] = value;
			return oldValue;
		}
		gdxAppError("ObjectMap","after if, Existing key was found.");
		i = -(i + 1); // Empty space was found.
		keyTable[i] = key;
		valueTable[i] = value;
		gdxAppError("ObjectMap","after assign keyTable and valueTable");
		if (++size >= threshold) {
			gdxAppError("ObjectMap","before resize(keyTable.length << 1);");
			resize(keyTable.length << 1);
		}
		gdxAppError("ObjectMap","nothing, so return null");
		return null;
	}
	...
	public @Null <T extends K> V get (T key) {
		gdxAppError("ObjectMap", "@Null <T extends K> V get (T key)");
		int i = locateKey(key);
	...
	public V get (K key, @Null V defaultValue) {
		gdxAppError("ObjectMap", " public V get (K key, @Null V defaultValue)");
		int i = locateKey(key);
		return i < 0 ? defaultValue : valueTable[i];
	}

	public @Null V remove (K key) {
		gdxAppError("ObjectMap", "public @Null V remove (K key) {");
		int i = locateKey(key);
		if (i < 0) return null;
		K[] keyTable = this.keyTable;
	...
	public boolean containsKey (K key) {
		gdxAppError("ObjectMap", "public boolean containsKey (K key)");
	...
	final void resize (int newSize) {
		gdxAppError("ObjectMap","resize (int newSize) called");
		...
		gdxAppError("ObjectMap","before instance keyTable and valueTable");
		...
		gdxAppError("ObjectMap","after resize (int newSize) called");
	}
